	...
	<?php 
		...
		$DeleteQuery = pg_prepare($dbconn, "my_query", "DELETE from gy19sml_markers WHERE oid=($1)"); 
		$result = pg_execute($dbconn, "my_query", array($oid))  or die ('<script> alert("Marker not deleted, incorrect MarkerID.") </script>');  

		
		if (is_null($result))	{
			echo '<script> alert("Marker not removed, Invalid MarkerID") </script>';
		}
		
		else {
			echo '<script> alert("Marker Removed.") </script>';
		}
