	...
	<?php 
		...
		$DeleteQuery = pg_prepare($dbconn, "my_query", "DELETE from gy19sml_comments WHERE oid=($1)"); 
		$result = pg_execute($dbconn, "my_query", array($oid))  or die ('<script> alert("Comment not removed, invalid CommentID.") </script>');  
		
		if (is_null($result))	{
			echo '<script> alert("Comment not removed, Invalid CommentID") </script>';
		}
		
		else {
			echo '<script> alert("Comment Removed") </script>';
		}
